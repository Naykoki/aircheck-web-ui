...
station_type = st.sidebar.selectbox("🏫 ประเภทสถานี", ["วัด", "โรงเรียน", "ชุมชน", "โรงพยาบาล", "อุตสาหกรรม"])
...
if "station" in st.session_state:
    folium.Marker(st.session_state.station, tooltip="🟢 จุดตรวจวัด", icon=folium.Icon(color="green")).add_to(m)

if "factories" in st.session_state:
    for i, f in enumerate(st.session_state.factories):
        folium.Marker(f, tooltip=f"🔴 โรงงาน {i+1}", icon=folium.Icon(color="red")).add_to(m)
        if "station" in st.session_state:
            dist = distance_km(st.session_state.station[0], st.session_state.station[1], f[0], f[1])
            folium.PolyLine([st.session_state.station, f], color="blue", tooltip=f"{dist:.2f} km").add_to(m)
...
def fetch_api(lat, lon, start_date, num_days):
    ...
    try:
        ...
        for i in range(total_hours):
            dt = datetime.combine(start_date, datetime.min.time()) + timedelta(hours=i)
            hour = dt.hour
...
if st.button("🚀 เริ่มจำลองข้อมูล"):
    if "station" not in st.session_state:
        ...
        st.stop()
    ref_df = fetch_api(st.session_state.station[0], st.session_state.station[1], start_date, num_days)
    if ref_df.empty:
        st.stop()
    rows = []
    ...
    st.download_button("📥 ดาวน์โหลด Excel", buf.getvalue(), file_name="AirCheckTH.xlsx")
